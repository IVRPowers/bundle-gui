 ...
 * 
 * $Id: rtsp_client.c,v 1.1 2015-10-20 09:15:15 borja.sixto Exp $
 */
...
static apt_bool_t rtsp_client_session_terminate_process(rtsp_client_t *client, rtsp_client_session_t *session)
{
  ...
  /* Voximal : Lock case with LumenVox, (send 407 after SPEAK) */
  {
    session->active_request = NULL;
    apt_log(APT_LOG_MARK,APT_PRIO_NOTICE,"Remove the active request, to send the TEARDOWN.");
  }
...
static apt_bool_t rtsp_client_task_msg_process(apt_task_t *task, apt_task_msg_t *task_msg)
{
	...
	apt_log(APT_LOG_MARK,APT_PRIO_INFO,"I6NET rtsp_client_task_msg_process %d", data->type);
