...
echo "<h2>"._("$userBrand Advanced Settings")."</h2>";
...
foreach ($conf as $c){
	...
	//Just to change the appearance of FreePBX in the settings text
	//$name_label_raw = $c['name'];
	$name_label_raw = str_replace("FreePBX",$userBrand,$c['name']);
	//$description_raw = $c['description'];
	$description_raw = str_replace("FreePBX",$userBrand,$c['description']);
