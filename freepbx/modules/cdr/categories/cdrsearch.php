...
if(isset($_POST['send'])) {
	...
	//$out = shell_exec("cp /var/spool/asterisk/monitor/* modules/cdr/tmpMonitor/");
	shell_exec("mkdir -p modules/cdr/tmpMonitor/");
	shell_exec("rm -f modules/cdr/tmpMonitor/*");
	shell_exec("ln -s /var/spool/asterisk/monitor/* modules/cdr/tmpMonitor/");
	shell_exec("chown -R asterisk. modules/cdr/tmpMonitor/*");
...
include_once("modules/cdr/crypt.php");
switch ($action) {
	case 'cdr_play':
	case 'cdr_audio':
  	include_once("modules/cdr/$action.php");
		exit;
		break;
	case 'download_audio':
		/*
			...
			$found=glob("/var/spool/asterisk/monitor/" . $_GET['cdr_file'] . "-*.wav");
			$file = $found[0];
			download_file($file, '', '', true);
			exit;
		break;
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		link.href = "modules/cdr/assets/soundmanager/demo/bar-ui/css/bar-ui.css";
		link.type = "text/css";
		link.rel = "stylesheet";
...
<script src="modules/cdr/assets/soundmanager/script/soundmanager2.js"></script>
<script src="modules/cdr/assets/soundmanager/demo/bar-ui/script/bar-ui.js"></script>
	<div id="maincdr">
	<table class="cdr">
	<tr><td>
...
<td align="right">
<a href="modules/cdr/categories/export_pdf.php" target="_blank"><img style="height: 30px; width: 30px;" src="modules/cdr/categories/images/pdf.gif" border="0">&nbsp;Export results as PDF</a>
</td>
<?php }?>
</tr>
</table>
...
<br>
<div id="player" style="display: none; text-align: center;">
...
if ($amp_conf['CEL_ENABLED'] && !isset($_POST['need_html']) && $action == 'cel_show') {
	...
	//$_SESSION["pdf_sql_query"] = "SELECT calldate, channel, src, clid, lastapp, lastdata, dst, dst, disposition, duration, userfield, accountcode FROM $db_name.$db_table_name $where";
...
//To pass the query to the pdf generation
if (isset($_POST['send'])) {
	$_SESSION["pdf_sql_query"] = "SELECT calldate, channel, src, clid, lastapp, lastdata, dst, dst, disposition, duration, userfield, accountcode FROM $db_name.$db_table_name $where";	
}
...
if ( $tot_calls_raw ) {
	...
	foreach($resultscdr as $row) {
		 ...
		 */
		$found=glob("/var/spool/asterisk/monitor/" . $row['uniqueid'] . "-*.wav");
		//$monitor_base = $amp_conf['MIXMON_DIR'] ? $amp_conf['MIXMON_DIR'] : $amp_conf['ASTSPOOLDIR'] . '/monitor';
		//$found=glob("$monitor_base" . $row['uniqueid'] . "-*.wav");
		if(count($found)>0){
			$recordingfile = $found[0];
		} else {
			$recordingfile = "";
		}
		/*
...
<script type="text/javascript">
function openPlayer(id,file) {

	file = file.replace("/var/spool/asterisk/monitor","modules/cdr/tmpMonitor");
	displayPlayer(file,id);

	//console.log($('#file').html());
	soundManager.reboot();
	soundManager.setup({
		url: "modules/cdr/assets/soundmanager/swf",
	 	flashVersion: 9,
	 	debugMode: false,
	  	onready: function() {
...
function displayPlayer(file,id) {
...
$(window).unload(function(){
	...
	jQuery.ajax({
		url: "modules/cdr/categories/player.php",
...
function cdr_formatRecordingFile($recordingfile, $basename, $id, $uid) {
	...
	if ($recordingfile) {
		...
		$recurl=$_SERVER['SCRIPT_NAME']."?quietmode=1&display=cdr&view=cdrsearch&action=cdr_play&recordingpath=$audio";
		//$download_url=$_SERVER['SCRIPT_NAME']."?display=cdr&action=download_audio&cdr_file=$uid";
		$download_url=$_SERVER['SCRIPT_NAME']."?display=cdr&view=cdrsearch&action=download_audio&cdr_file=$uid";
		$playbackRow = $id +1;
		//
		...
		//<a href=\"$download_url\" target='_blank'><img src=\"assets/cdr/images/cdr_download.png\" alt=\"Call recording\" /></a></td>";
		//echo "<td title=\"$basename\"><a href=\"#\" onClick=\"openPlayer('$uid','$recordingfile');\"><img src=\"assets/cdr/images/cdr_sound.png\" alt=\"Call recording\" /></a>
		//<a href=\"$download_url\" target='_blank'><img src=\"assets/cdr/images/cdr_download.png\" alt=\"Call recording\" /></a></td>";
		echo "<td title=\"$basename\"><img src=\"assets/cdr/images/cdr_sound.png\" style='cursor: pointer' onClick=\"openPlayer('$uid','$recordingfile');\" alt=\"Call recording\" />
		<a href=\"$download_url\" target='_blank'><img src=\"assets/cdr/images/cdr_download.png\" alt=\"Call recording\" /></a></td>";
