...
class Dashboard extends FreePBX_Helpers implements BMO {
				...
				if(!empty($order)) {
					foreach($config as &$page) {
						...
						foreach($page['entries'] as $k => $e) {
							...
							$o = isset($order[$e['section']]) ? $order[$e['section']] : ($k + 1); //Modified to be able of adding new entries
							$entries[$o] = $e;
						}
						ksort($entries);
	...
	private function getAjaxVoiceXML() {
		if (!class_exists('VoiceXML')) {
			include 'classes/VoiceXML.class.php';
		}
		$tmp = new VoiceXML();
		return $tmp->getHTML();
	}
