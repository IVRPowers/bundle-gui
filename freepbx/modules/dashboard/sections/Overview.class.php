...
class Overview {
	...
	private function getNotifications($showall = false) {
		...
		//What we do here is to get rid of the messages that we don't care about
		$itemsTemp = array();
		for ($i = 0; $i < count($items); $i++) {
			if ($items[$i]['id'] == "BROWSER_STATS") continue;
			elseif ($items[$i]['id'] == "NOEMAIL") continue;
			elseif ($items[$i]['id'] == "NEWUPDATES") continue;
			elseif ($items[$i]['id'] == "ASTCONFAPPMISSING") continue;
			elseif ($items[$i]['id'] == "SIGNATURE_CHECK") continue;
			elseif ($items[$i]['id'] == "MISSING_CALLRECORDINGS") continue;
			elseif ($items[$i]['id'] == "SYMLINKNOTICE") continue;
			else $itemsTemp[] = $items[$i];			
		}
		
		$items = $itemsTemp;
		/*	
		foreach ($items as $element) {
			error_log(print_r($element,true), 0);
		}*/
		...
		$searchArray = array("Inbound Route","Trunk");
		$replaceArray = array("Number","SIP Trunk");
		foreach ($items as $notification) {
			$notification["extended_text"] = str_replace($searchArray,$replaceArray,$notification["extended_text"]);
			$final['nots'][] = array(
				"id" => $notification['id'],
	...
	public function getSummary() {
		$svcs = array(
			"asterisk" => _("Telephony"),
			"mysql" => _("Database"),
			"apache" => _("Web Server"),
			"openvxi" => _("Interpreter"),	
		);
	...
	private function checkasterisk($sysinfo) {
		if (!isset($sysinfo['ast.uptime.system-seconds'])) {
			return $this->genAlertGlyphicon('critical', 'Unable to find Telephony results');
		}
		$ast = $sysinfo['ast.uptime.system-seconds'];

		// Check to see if Asterisk is up and running.
		if ($ast == -1) {
			return $this->genAlertGlyphicon('error', 'Telephony not running');
		}

		// Can we connect to asterisk?
		if ($ast == -2) {
			return $this->genAlertGlyphicon('critical', 'Telephony Manager Interface (astman) failure');
		}

		$uptime = $sysinfo['ast.uptime.system'];
		// Up for less than 10 minutes? Is it crashing?
		if ($ast < 600) {
			return $this->genAlertGlyphicon('warning', "Telephony running for less than 10 minutes ($uptime)");
		}

		return $this->genAlertGlyphicon('ok', "Telephony uptime $uptime");
	}
	...
	private function checkapache() {
		...
		// say yes.
		return $this->genAlertGlyphicon('ok', "Web Server running");
	}
	
	private function checkopenvxi() {
		
		//TODO Preguntar comprobaciones a Borja
		
		$out = shell_exec("pidof openvxi | wc -w");
		if ($out == 0) {
			return $this->genAlertGlyphicon('error', 'VoiceXML Interpreter not running');
		}
		
		
		return $this->genAlertGlyphicon('ok', "VoiceXML Interpreter running");
